

Язык Си. Базовые типы. Представление значений вещественных базовых типов в памяти. Диапазоны значений вещественных базовых типов.

Язык Си. Вещественные типы. Операции языка Си над значениями вещественных типов. Преобразования «по умолчанию» между вещественными типами языка Си.

Язык Си. Выражения. Порядок вычисления выражения в языке Си. Приоритет и ассоциативность операции.

Язык Си. Стандартные функции языка Си для работы с динамической памятью.

Язык Си. Понятие времени жизни и области видимости переменных. Глобальные и локальные переменные. Модификаторы области видимости и времени жизни

Язык Си. Понятие указателя в языке Си. Операции над указателями.

Язык Си. Понятие функции. Описание функций в Си. Передаваемые параметры. Передача параметров через стек вызовов. Возвращаемые значения.

Язык Си. Нормализованное представление вещественных чисел, мантисса и порядок. Двоичное представление значений вещественных типов, особенности вещественной арифметики.

Язык Си. Препроцессор. Макроопределения.

Язык Си. Препроцессор. Включаемые файлы. Условная компиляция.

Массивы. Многомерные массивы. Индексация многомерных массивов. Распределение памяти в многомерных массивах. Связь понятия указателя и массива. Инициализаторы массивов.


Стандартные функции языка Си для работы файлами fopen, fclose, feof, fprintf, fscanf, fread, fwrite. Потоки stdin, stdout, stderr.

Стандартные функции языка Си для форматированного ввода-вывода. Форматная строка (целые знаковые и беззнаковые в десятичном и шестнадцатеричном виде, числа с плавающей точкой, буквы, строки). Возвращаемое значение


Строки в языке Си. Понятие длины строки. Инициализаторы строк. Функции работы со строками: определение длины строки, копирование строк, слияние строк.

Структуры. Синтаксис описания структур. Обращение к полям структур для объектов и к полям по указателю на объект типа структура. Инициализатор структур.

Функции с переменным числом параметров. Получение значений, передаваемых после фиксированных параметров.

Целочисленная машинная арифметика. Перенос и переполнение. Дополнительный код. Знаковый разряд.

Целочисленные типы языка Си. Операции языка Си над значениями целочисленных типов. Преобразования «по умолчанию» между целочисленными типами языка Си.


---


Алгоритм Беллмана-Форда построения кратчайшего пути в графе. Оценка сложности.

Алгоритм Бойера-Мура поиска подстроки в строке без суффиксной эвристики. Оценка сложности.

Алгоритм Рабина-Карпа поиска подстроки в строке. Оценка сложности.

Алгоритм топологической сортировки. Оценка сложности.

Алгоритмы записи целых чисел в произвольной b-с.с.

Алгоритмы обхода графа в глубину и в ширину. Оценки сложности.

Алгоритмы обхода деревьев (в ширину, в глубину: в пре/пост/инфиксном порядке).

Алгоритмы перевода записи вещественных чисел из с.с. в с.с.

Алгоритмы перевода записи целых чисел из с.с. в с.с.

Алгоритмы поиска значения в списке и добавления значения в список. Реализация списков с помощью массивов и с помощью структур.

Алгоритмы с возвратом на примере расстановки ферзей.

Алгоритмы сортировки массива простым включением, простым выбором, «пузырьком». Оценка сложности.

Алгоритм Краскала построения каркаса графа (без СНМ). Оценка сложности.

Алгоритм Прима каркаса графа. Оценка сложности.

Алгоритм быстрой сортировки Хоара. Оценка сложности.

Алгоритм обхода графа в ширину с использованием очереди.

Алгоритм пирамидальной сортировки. Оценка сложности.

Алгоритм Флойда-Уоршелла вычисления длин кратчайших путей между всеми парами вершин. Оценка сложности.

Алгоритм Дейкстры поиска кратчайших путей в графе. Оценка сложности.

Алгоритм вставки элемента в АВЛ-дерево. Длинный и короткий повороты. Оценка сложности.

Граф. Представление в ЭВМ: матрицы смежности и инцидентности, список дуг.

Транзитивное замыкание графа. Алгоритм Флойда-Уоршелла.

Дерево двоичного поиска, алгоритмы включения и удаления без балансировки, сравнение их сложности с известными алгоритмами на массивах.

Дерево, как частный вид графа. Способы представления деревьев в ЭВМ: левая и правая скобочная запись, список предков, указатели.

Способы представления деревьев в ЭВМ: левое и правое скобочная запись, список предков, указатели.

Динамическое программирование на примере конкретной задачи. Обратный ход.

Метод Хаффмана построения оптимального префиксного двоичного кода.

Методы простого поиска в массиве: линейный поиск, бинарный поиск, оценки сложности.

Основные операции со стеком и очередью, способы реализации стека и очереди с помощью массивов и с помощью структур.

Система непересекающихся множеств, операции определения представителя множества и объединения двух множеств. Реализация операций на Си на основе дерева с рангом, дерева с рангом и сжатием путей.

Список, как линейно упорядоченная структура данных последовательного доступа. Одно- и двусвязные списки.

Стек, преобразование инфиксной формы записи выражения в постфиксную и вычисление значения полученного выражения.


