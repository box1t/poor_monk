...
# Провера количества переданных параметров при вызове: минимум 2
if [ "$#" -lt 2 ]; then
    echo "Expected 2 params : $0 <FILE_NAME ~ hello.c> <PROCESS_AMOUNT ~ uint8 > 0>"
    exit 1
fi
...
# Проверка ввода 2ого параметра: введено целое число больше нуля
if ! [[ "$2" =~ ^[1-9][0-9]*$ ]]; then
    echo "Wrong 2nd param input: Expected: <(uint8) > 0>, Got: ($2)."
    exit 1
fi
...
# Function to execu# Function to execute on Ctrl+C
cleanup() {
...
mpirun -np $PROCESS_AMOUNT ./UTILS/compiled_code $@
EOF
#===========
