...
import numpy as np # Добавляем numpy для потенциального сглаживания, хотя прямые линии для ω̂(t) тоже подходят
...
def task1(N:int, m:int=30, dt:int=100, out_prefix:Path=Path("task1_variant")):
    ...
    # Собираем данные, включая t_start и t_end для plt.bar
    t_starts = []
    t_mids = []
    omegas = []
    ...
    for i,(t0,t1) in enumerate(intervals, start=1):
        t_mid = 0.5*(t0+t1)
        rows.append({"k":i,"t_start":t0,"t_end":t1,"t_mid":t_mid,"Δn":dn[i-1],"ω̂":omega[i-1]})
        t_starts.append(t0)
        t_mids.append(t_mid)
        omegas.append(omega[i-1])

    csv_path = out_prefix.with_suffix(".csv")
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        w=csv.DictWriter(f, fieldnames=rows[0].keys()); w.writeheader(); w.writerows(rows)
    
    plt.figure(figsize=(10, 6)) # Увеличим размер фигуры для лучшей читаемости
    
    # 1. Гистограмма (столбцы)
    # Используем t_start как левый край столбца, а dt как ширину
    plt.bar(t_starts, omegas, width=dt, align='edge', edgecolor='black', alpha=0.7, label='ω̂(t) (интервалы)')
    
    # 2. Сглаженная линия
    # Просто соединяем центры столбцов
    plt.plot(t_mids, omegas, linestyle='-', marker='o', color='red', linewidth=2, label='ω̂(t) (сглажено)')
    
    plt.xlabel("t, ч")
    plt.ylabel("ω̂(t), 1/ч на комплект")
    plt.title("Оценка потока отказов ω̂(t) (Гистограмма)")
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.legend() # Добавляем легенду, чтобы было понятно, где гистограмма, а где линия
    plt.tight_layout()
    png_path = out_prefix.with_name(out_prefix.name + "_omega_smoothed_hist.png"); plt.savefig(png_path); plt.close()
